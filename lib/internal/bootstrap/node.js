...
const startupTraceMark = globalThis.__edgeStartupTraceMark;
function markBootstrapNodeSubphase(name) {
  if (typeof startupTraceMark === 'function') {
    startupTraceMark(name);
  }
}

const config = internalBinding('config');
markBootstrapNodeSubphase('top-level.internalBinding-config');
const internalTimers = require('internal/timers');
markBootstrapNodeSubphase('top-level.require-internal-timers');
const { defineOperation } = require('internal/util');
markBootstrapNodeSubphase('top-level.require-internal-util');
const {
  validateInteger,
} = require('internal/validators');
markBootstrapNodeSubphase('top-level.require-internal-validators');
const {
  constants: {
    kExitCode,
    kExiting,
...
markBootstrapNodeSubphase('top-level.internalBinding-util');

setupProcessObject();
markBootstrapNodeSubphase('setup-process-object');

setupGlobalProxy();
markBootstrapNodeSubphase('setup-global-proxy');

setupBuffer();
markBootstrapNodeSubphase('setup-buffer');
markBootstrapNodeSubphase('setup-process-global-buffer');
...
markBootstrapNodeSubphase('process-exit-state');
...
markBootstrapNodeSubphase('process-config');

require('internal/worker/js_transferable').setup();
markBootstrapNodeSubphase('js-transferable');
...
markBootstrapNodeSubphase('process-methods');
...
markBootstrapNodeSubphase('posix-credentials');
...
markBootstrapNodeSubphase('async-hooks');
...
markBootstrapNodeSubphase('task-queues-and-timers');
...
markBootstrapNodeSubphase('trace-category-handler');
...
markBootstrapNodeSubphase('allowed-node-environment-flags');
...
markBootstrapNodeSubphase('process-features');
...
markBootstrapNodeSubphase('fatal-exception-hooks');
...
markBootstrapNodeSubphase('warning-and-typescript-flags');
...
markBootstrapNodeSubphase('timers-activation');
...
markBootstrapNodeSubphase('source-map-support');

{
  const { getBuiltinModule } = require('internal/modules/helpers');
  process.getBuiltinModule = getBuiltinModule;
}
markBootstrapNodeSubphase('builtin-module-helper');

function setupProcessObject() {
  markBootstrapNodeSubphase('setup-process-object.begin');
  const EventEmitter = require('events');
  markBootstrapNodeSubphase('setup-process-object.require-events');
  const origProcProto = ObjectGetPrototypeOf(process);
  ObjectSetPrototypeOf(origProcProto, EventEmitter.prototype);
  markBootstrapNodeSubphase('setup-process-object.set-prototype');
  FunctionPrototypeCall(EventEmitter, process);
  markBootstrapNodeSubphase('setup-process-object.init-emitter');
  ...
  markBootstrapNodeSubphase('setup-process-object.define-process-tag');
  ...
  markBootstrapNodeSubphase('setup-process-object.install-global-process');
}
...
function setupBuffer() {
  markBootstrapNodeSubphase('setup-buffer.begin');
  const {
    Buffer,
  } = require('buffer');
  markBootstrapNodeSubphase('setup-buffer.require-buffer');
  const bufferBinding = internalBinding('buffer');
  markBootstrapNodeSubphase('setup-buffer.internal-binding');

  // Only after this point can C++ use Buffer::New()
  bufferBinding.setBufferPrototype(Buffer.prototype);
  markBootstrapNodeSubphase('setup-buffer.set-buffer-prototype');
  ...
  markBootstrapNodeSubphase('setup-buffer.install-global-buffer');
}
